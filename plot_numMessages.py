...
num_messages = [25, 50, 75, 100, 150, 200]
...
for time in startTime:
    ...
    for num_msg in num_messages:
        ...
        path_to_metrics = path + "LE_" + str(time) + "_" + str(T) + "/" + "XChant/ALL/mules_" + \
                          str(num_mules) + "/channels_" + str(num_channel) + "/P_users_" + str(num_Puser) + \
                          "/" + str(num_msg) + "/metrics.txt"
...
plt.xticks(np.arange(25, 205, 25))
# plt.xlim(0,12)
fig_name = "dummy.eps"
...
if p_id == 1:
    ...
    plt.ylim(-0.05,1)
    fig_name = "Plots/pdr_num_msg_UMass.eps"
...
if p_id == 2:
    ...
    fig_name = "Plots/latency_num_msg_UMass.eps"

if p_id == 3:
    plt.ylabel('Energy expenditure (KJ)', fontsize=25)
    plt.xlabel('Number of messages', fontsize=25)
    plt.ylim(-1, 13)
    fig_name = "Plots/energy_num_msg_UMass.eps"

if p_id == 4:
    plt.ylabel('Message overhead', fontsize=25)
    plt.xlabel('Number of messages', fontsize=25)
    plt.ylim(-1, 36)
    fig_name = "Plots/overhead_num_msg_UMass.eps"
...
if p_id == 1:
    plt.legend([ "dDSAaR", "dDSAaR (Ideal)"], loc="lower center", fontsize=15, ncol = 3, frameon=False)
elif p_id == 2:
